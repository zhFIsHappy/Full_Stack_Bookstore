...
public class BookDaoJdbc implements BookDao {
    ...
    public List<Book> findByCategoryId(long categoryId) {
        ...
        try (Connection connection = JdbcUtils.getConnection();
            ...
            statement.setLong(1, categoryId); // 1 is for the position of the ?
            try (ResultSet resultSet = statement.executeQuery()) {
    ...
    public List<Book> findRandomByCategoryId(long categoryId, int limit) {
        ...
        List<Book> books = new ArrayList<>();
        List<Book> randomized = new ArrayList<>();
        try (Connection connection = JdbcUtils.getConnection();
        ...
        while(limit != 0){
            int randomIndex = ThreadLocalRandom.current().nextInt(books.size());
            limit --;
            randomized.add(books.get(randomIndex));
